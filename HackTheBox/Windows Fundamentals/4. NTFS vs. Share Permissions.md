...
![[Pasted image 20240904134745.png]]

NTFS permissions and share permissions are often understood to be the same. Please know that they are not the same but often apply to the same shared resource. L
## Share permissions
...
![[Pasted image 20240904135523.png]]
...
```shell-session
Precious@htb[/htb]$ smbclient -L SERVER_IP -U htb-student
...
```shell-session
Precious@htb[/htb]$ smbclient '\\SERVER_IP\Company Data' -U htb-student
...
![[Pasted image 20240904140207.png]]
...
```bash
Precious@htb[/htb]$ sudo mount -t cifs -o username=htb-student,password=Academy_WinFun! //ipaddoftarget/"Company Data" /home/user/Desktop/
```
...
```bash
Precious@htb[/htb]$ sudo apt-get install cifs-utils
```
...
![[Pasted image 20240904140026.png]]
