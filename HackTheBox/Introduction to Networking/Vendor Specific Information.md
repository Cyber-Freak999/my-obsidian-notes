...
```shell-session
Precious@htb[/htb]$ telnet 10.129.10.2
...
![[Pasted image 20240909021902.png]]
...
```shell-session
Precious@htb[/htb]$ sudo modprobe 8021q
```
...
```shell-session
Precious@htb[/htb]$ lsmod | grep 8021
...
```shell-session
Precious@htb[/htb]$ sudo vconfig add eth0 20
...
```shell-session
Precious@htb[/htb]$ sudo ip addr add 192.168.1.1/24 dev eth0.20
Precious@htb[/htb]$ sudo ip link set up eth0.20
```
...
```shell-session
Precious@htb[/htb]$ ip a | grep eth0.20
...
![[Pasted image 20240909021954.png]]
...
![[Pasted image 20240909022011.png]]
...
![[Pasted image 20240909022029.png]]
...
```shell-session
Precious@htb[/htb]$ tshark -r "The Ultimate PCAP v20221220.pcapng" -T fields -e vlan.id | sort -n -u
...
![[Pasted image 20240909022041.png]]
...
![[Pasted image 20240909022056.png]]
