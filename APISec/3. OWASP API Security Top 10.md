...
## Description
Most common and damaging API vulnerability. Manipulation of APIs to access data/objects belonging to other users. Cant be detected on runtime.
Attacker authenticates as User A and then retrieves data on User B is one major example.
## Risk Exposure
Can lead to data loss, disclosure, data manipulation.
## Prevention
- Discuss auth rules during API design phase
...
## Prevention
- Define authentication policies and standards based on business requirements
- Follow best practices; dont assume apis are hidden
...
> Attackers will evade detection by limiting request velocity
...
- Train developers to consider non "happy path" usage
...
- Identify internally developed APIs as well as 3rd party ones too
...
## Description
Exposures can occur via use of 3rd party APIs which are generally trusted. However, they can be exploited, which can be used to attack APIs that rely on them(supply chain attacks)
Examples:
