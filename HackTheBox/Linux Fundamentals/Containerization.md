...
```shell-session
[!bash!]$ docker run -p <host port>:<docker port> -d <docker container name>
```

#### Docker Run

```shell-session
[!bash!]$ docker run -p 8022:22 -p 8080:80 -d FS_docker
```
...
```shell-session
[!bash!]$ sudo apt-get install lxc lxc-utils -y
```
...
```shell-session
[!bash!]$ sudo lxc-create -n linuxcontainer -t ubuntu
```
...
```shell-session
[!bash!]$ sudo vim /usr/share/lxc/config/linuxcontainer.conf
```
...
```shell-session
[!bash!]$ sudo systemctl restart lxc.service
```
